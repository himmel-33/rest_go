package coms //th4 에서 지역 컬럼 주가하여 여러사이트로 관리 가능하게 만듬
...
import ( //mDBConnSelect.Query for문 통해 추가될 타입만큼 검색 가능 / 구조체(json) 배열이나 map 에 넣기 위해서는 interface{} 사용
	...
	_ "github.com/go-sql-driver/mysql" //설치명령어 go get github.com/go-sql-driver/mysql(설치되면 go.sum 파일이 생김)
)
...
func runResever(conf *GatewayDBInfo) {
	...
	r.GET(":location/data", func(c *gin.Context) { //3개 가스 최신 데이터

		strtype := c.Param("location")
		...
		for _, tyname := range types {
			rows, err1 := mDBConnSelect.Query("SELECT type, pressure, differential, date FROM gas where type = ? and location = ? order by date desc limit 1", tyname, location)
			if err1 == nil {
				for rows.Next() {
	...
	r.GET(":location/data/:type", func(c *gin.Context) { //특정 가스의 최신 데이터

		strtype1 := c.Param("location")
		strtype2 := c.Param("type")
		...
		if strtype1 == "seoul" {
			location = 119
		} else if strtype1 == "busan" {
			location = 124
		}

		rows, err1 := mDBConnSelect.Query("SELECT type, pressure, differential, date FROM gas where type = ? and location = ? order by date desc limit 1", strtype2, location)
		if err1 == nil {
			for rows.Next() {
				err1 := rows.Scan(&type1, &differential, &pressure, &date)
				if err1 != nil {
	...
	r.GET(":location/data_all", func(c *gin.Context) { //가스 모든 데이터
		...
		rows, err1 := mDBConnSelect.Query("SELECT type, pressure, differential, date FROM gas where location = ?", location)
		if err1 == nil {
			for rows.Next() {
				err1 := rows.Scan(&type1, &differential, &pressure, &date)
				if err1 != nil {
	...
	r.GET(":location/data_all/:type", func(c *gin.Context) { //특정 가스의 모든 데이터
		...
		rows, err1 := mDBConnSelect.Query("SELECT type, pressure, differential, date FROM gas where type =? and location =?", strtype2, location) //tankinfo
		if err1 == nil {
			for rows.Next() {
				err1 := rows.Scan(&type1, &differential, &pressure, &date)
				if err1 != nil {
