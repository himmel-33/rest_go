// package main //main 일 겨우 실행 프로그램으로 만듬  //json 구조체에 값을 넣어 출력하지 않고 그냥 배열 a[]에 넣어 값출력(json에 넣으면 c.json 때문에 data가 두번출력)
...
// 		rows, err1 := mDBConnSelect.Query("SELECT JSON_OBJECT('type', type,'pressure', pressure, 'differential', differential) FROM gas where type = 'Argon' order by date desc limit 1")
// 		if err1 == nil {
// 			for rows.Next() {
// 				err1 := rows.Scan(&data)
// 				if err1 != nil {
...
// 			}
// 		}
// 		rows2, err2 := mDBConnSelect.Query("SELECT JSON_OBJECT('type', type,'pressure', pressure, 'differential', differential) FROM gas where type = 'LiquidNitrogen' order by date desc limit 1")
// 		if err2 == nil {
// 			for rows2.Next() {
// 				err2 := rows2.Scan(&data)
// 				if err2 != nil {
// 					log.Println(err2)
// 				} else {
// 					a["nitrogen"] = data
...
// 			}
// 		}
// 		rows3, err3 := mDBConnSelect.Query("SELECT JSON_OBJECT('type', type,'pressure', pressure, 'differential', differential) FROM gas where type = 'LiquidOxygen' order by date desc limit 1")
// 		if err3 == nil {
// 			for rows3.Next() {
// 				err3 := rows3.Scan(&data)
// 				if err3 != nil {
// 					log.Println(err3)
// 				} else {
// 					a["oxygen"] = data
// 					fmt.Println(data)
// 					//\ 역슬래쉬 빼는 부분
// 					byt, _ := json.Marshal(a)                           //마샬링(값 byte타입변환)
// 					body := strings.Replace(string(byt), `\"`, `"`, -1) //1.역슬래시 제거
// 					body = body[1 : len(body)-1]                        //2.양쪽 끝의 '"'쌍따옴표 제거
// 					body = strings.Replace(body, `\\`, `\`, -1)         //3.2개의 역슬래시를 하나로 줄이기
// 					var data1 map[string]interface{}                    //4.언마샬링(원상복귀)
